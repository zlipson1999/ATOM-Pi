...
4. Verify it loads:
   ...
   ```
   Wake model loaded — say: 'hey atom'
   Mic at 16000 Hz (native). Ctrl+C to stop.
   ```
   The rate line reports your own hardware — `resampling` instead of
   `native` is normal for USB mics that refuse 16 kHz. The listener is
   then armed and silent: it prints nothing more until it hears the
   phrase. `Re-armed. Listening for wake word...` appears only *after*
   a completed wake cycle, so not seeing it at startup is correct.
...
```
`wakeword_validate.py` is the repo's `validate_model.py`, renamed by
the installer when it copies it to `~/pocket-ai`. Running it from a
checkout of the repo instead? Use `python validate_model.py` there.
...
`throttled != 0x0` means trouble — and it flags BOTH heat and
undervoltage. Since the Pi now powers the display too, undervoltage is
the first suspect: turn the display brightness down and re-test before
blaming cooling.
