...
#  continues where it left off. Expect: run -> reboot -> run -> reboot
#  -> run -> done.
# =============================================================================
set -u
...
if [ "$(get_stage)" -eq 1 ]; then
  ...
  [ "$HAILO_SEEN" -ge 1 ] || die "Hailo not detected on PCIe. Power off and check: ribbon cable seated+latched both ends, correct orientation, pogo/supplemental power connected, module screwed down flat. Then run the installer again."
  [ "$NVME_SEEN" -ge 1 ] || die "NVMe drive not detected on PCIe. Power off and reseat the SSD in its slot; check the board's supplemental power. Then run the installer again."
...
if [ "$(get_stage)" -eq 3 ]; then
  ...
  # Audio output: prefer the touchscreen's HDMI speakers, else first USB
  CARD=$(aplay -l 2>/dev/null | grep -m1 -oP 'card \K\d+(?=: vc4hdmi)')
  [ -z "$CARD" ] && CARD=$(aplay -l 2>/dev/null | grep -im1 usb | grep -oP 'card \K\d+')
  if [ -n "$CARD" ]; then TTS_DEV="plughw:${CARD},0"; else TTS_DEV="default"; warn "no HDMI/USB audio found yet — using ALSA default (edit TTS_ALSA_DEVICE in .env later)"; fi
  cat > .env <<ENVEOF
PORT=8000
...
Path=$APP_DIR
EOF
  cp "$HOME/.config/autostart/atom.desktop" "$HOME/Desktop/" 2>/dev/null || true
...
bold "Installer already completed (stage $(get_stage))."
echo "To force a full re-run:  rm $STAGE_FILE  and run again."
