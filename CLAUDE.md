...
1. **install.sh is the installer. Run it; do not rewrite it.**
   ...
   Pi at stage 0, again at stage 1 the first time PCIe Gen 3 is
   enabled (true on any stock image), and again at stage 2 — three
   automatic reboots on a fresh build, plus a manual restart if the
   microSD-to-NVMe migration is still pending. After each one, run
   `bash install.sh` again and it continues. Fix failures minimally
   and re-run; do not invent an alternative install path.
   On a COMPLETED install (stage 5), re-running performs a fast sync
   of the ATOM layer instead — that is the supported way to add a
   `hey_atom.onnx` trained after the fact. `bash install.sh --sync`
