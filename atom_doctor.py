...
def run(args, timeout=20):
    """Run an argument list with NO shell. Used wherever a value from .env or
    the environment is involved (TTS_ALSA_DEVICE, LOG_FILE) — those are
    user-editable, and interpolating them into a shell string let a stray
    semicolon or backtick execute."""
    try:
...
def checks():
    ...
    results = []
    model = (
        Path("/proc/device-tree/model").read_text(errors="ignore")
    ...
    results.append(
        row(
            "Raspberry Pi",
            "OK" if "Raspberry Pi 5" in model else "DEGRADED",
            model.strip("\x00") or "not a Pi?",
        )
    )
    thr = sh(["vcgencmd", "get_throttled"]).strip()
    results.append(
        row("Cooling/power", "OK" if "0x0" in thr else "DEGRADED", thr or "vcgencmd unavailable")
    )
    ...
    results.append(
        row(
            "Boot drive",
            "OK" if "nvme" in root else "DEGRADED",
            root + ("" if "nvme" in root else " (SD card — slower)"),
        )
    )
    hail = sh(["hailortcli", "fw-control", "identify"])
    results.append(
        row(
            "Hailo accelerator",
            "OK" if "Board" in hail or "Hailo" in hail else "MISSING",
            "detection off if missing",
        )
    )
    ...
    results.append(
        row(
            "Wake model",
            "OK" if (APP / "hey_atom.onnx").exists() else "MISSING",
            "present — mic validation per VALIDATION.md still required"
            if (APP / "hey_atom.onnx").exists()
            else "'Hey ATOM' off — mic button works",
        )
    )
    ...
    results.append(
        row(
            "Voice model",
            "OK" if voice else "DEGRADED",
            "" if voice else "falls back to default US voice",
        )
    )
    ...
    results.append(
        row(
            "Brain model (GGUF)",
            "OK" if brain else "MISSING",
            "" if brain else "downloads at first start",
        )
    )
    oll = sh(["ollama", "list"])
    results.append(
        row(
            "Vision (Moondream)",
            "OK" if "moondream" in oll else "MISSING",
            "'what do you see' needs it",
        )
    )
    try:
        ...
        ok = False
    results.append(
        row("Backend", "OK" if ok else "MISSING", "" if ok else "start it: bash start-atom.sh")
    )
    ...
    try:
        ...
        results.append(
            row(
                "Local library",
                "OK" if "LIBRARY: /" in st else "DEGRADED",
                ("optional — " if "LIBRARY: /" not in st else "") + st.replace("LIBRARY: ", ""),
            )
        )
    ...
    if "MISSING" in results:
        print("  STATUS: DEGRADED — items above are missing")
    elif "DEGRADED" in results:
        print("  STATUS: READY (with notes above)")
    else:
        print("  STATUS: READY (bench check — physical validation per VALIDATION.md still applies)")


def sound():
    dev = os.environ.get("TTS_ALSA_DEVICE", "default")
    print(f"Playing test tone on {dev} — you should hear 'front left/right'...")
    subprocess.run(["speaker-test", "-D", dev, "-c", "2", "-t", "wav", "-l", "1"])


def mic():
    print("Recording 3 seconds — say: 'Can you hear me, ATOM?'")
    subprocess.run(["arecord", "-d", "3", "-f", "cd", "/tmp/atom_mic_test.wav"])
    dev = os.environ.get("TTS_ALSA_DEVICE", "default")
    print("Playing it back...")
    subprocess.run(["aplay", "-D", dev, "/tmp/atom_mic_test.wav"])
...
def backup():
    ...
    with tarfile.open(out, "w:gz") as t:
        for f in [".env", "personality.txt", "hey_atom.onnx", "conversations.json", "tools.json"]:
            if (APP / f).exists():
                t.add(APP / f, arcname=f)
    print(f"Backup written: {out}\nRestore: extract into ~/pocket-ai after reinstalling.")


def logs():
    lf = (APP / os.environ.get("LOG_FILE", "atom.log")).resolve()
    if APP.resolve() not in lf.parents:
        print("Refusing to read a log outside the ATOM application directory.")
        return
    print(run(["tail", "-n", "60", str(lf)]) or f"(no log at {lf} yet)")
...
def version():
    ...
    print(
        "  Wake model:     ",
        "hey_atom.onnx" if (APP / "hey_atom.onnx").exists() else "NOT INSTALLED",
    )
    print(
        "  Voice model:    ",
        "en_GB-alan-medium"
        if (APP / "models/en_GB-alan-medium.onnx").exists()
        else "stock default",
    )
