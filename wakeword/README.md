...
Copy `hey_atom.onnx` to `~/pocket-ai/hey_atom.onnx` (or drag it into
the atom-pi GitHub repo root and re-run the installer). Put the
validated threshold in `.env`. Then the REAL test — live, in the
robot's actual room, on its actual mic:
